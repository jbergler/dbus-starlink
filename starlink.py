...
class Dishy:
    ...
    def get_position(self):
        ...
        if r.status.code != 0:
            logging.warning(
                f"Starlink location request failed: {r.status.message} (code: {r.status.code})"
            )
...
class DbusService:
    def __init__(self, target="192.168.100.1:9200"):
        ...
        self._dbusservice.add_path(
            "/FirmwareVersion",
            info.software_version if info.HasField("software_version") else "N/A",
        )
        self._dbusservice.add_path(
            "/HardwareVersion",
            info.hardware_version if info.HasField("hardware_version") else "N/A",
        )
